...
namespace Nethermind.Store
{
    ...
    {
        private static readonly LruCache<ValueKeccak, Rlp> NodeCache = new LruCache<ValueKeccak, Rlp>(64 * 1024);
...
//            ValueCache.Delete(rawKey);
        ...
        internal Rlp GetNode(ref ValueKeccak keccak, bool allowCaching)
        {
            if (!allowCaching)
            {
                return new Rlp(_db[keccak.BytesAsSpan.ToArray()]);
            }
            
            return NodeCache.Get(keccak) ?? new Rlp(_db[keccak.BytesAsSpan.ToArray()]);
        }
